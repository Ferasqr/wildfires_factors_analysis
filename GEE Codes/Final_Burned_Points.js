...
var Continent = ee.FeatureCollection("projects/ee-burned/assets/Australia");
...
//***********************************\\MODIS MASK BURNED AREAS//****************************************//
  ...
  var Modis = dataset.filter(ee.Filter.date('2010-01-01', ee.Date(Date.now()))); //ee.Date(Date.now()
...
//***********************************\\VARAIBLES//****************************************//
  ...
  var endDate = ee.Date(Date.now());  //Date.now()
...
//////TOPOGRAPHIC VARIABLES//////
  ...
  var mcd12q1 = ee.ImageCollection('MODIS/061/MCD12Q1')
                    .filter(ee.Filter.date('2010-01-01', Date.now()))
                    .first();
...
//USING A FOR LOOP TO REACH EACH SINGLE HEXAGON AND THE POINTS WITHIN EACH HEXAGON
  ...
  folder: 'Finalcsv',
  description: 'Final_Burned_Points', 
  fileFormat: 'CSV' 
});
